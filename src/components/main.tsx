...
function countNeighboursWrap(
  ...
  cols.forEach(x => {
    if (cells[above][x]) count++
  })
  cols.forEach(x => {
    if (cells[below][x]) count++
  })
...
function countNeighboursNoWrap(
  cells: Board,
  gridSize: number,
  i: number,
  j: number
  ...
  if (above)
    cols.forEach(x => {
      if (cells[i - 1][x]) count++
    })
  if (below)
    cols.forEach(x => {
      if (cells[i + 1][x]) count++
    })
  if (left) if (cells[i][j - 1]) count++
  if (right) if (cells[i][j + 1]) count++
...
function Main() {
  ...
  // Settings
  const [gridSize, setGridSize] = useStorage(
    'golSize',
    defaultGridSize,
    localStorage
  )
  const [gridScale, setGridScale] = useStorage(
    'golScale',
    defaultGridScale,
    localStorage
  )
  const [randomizeChance, setRandomizeChance] = useStorage(
    'golRandomizeChance',
    defaultRandomize,
    localStorage
  )
  const [wrapEdges, setWrapEdges] = useStorage(
    'golWrapEdges',
    defaultWrapEdges,
    localStorage
  )
  ...
  const [patterns, setPatterns] = useStorage<Pattern[]>(
    'golSaved',
    defaultPatterns,
    localStorage
  )
  ...
  useEffect(() => {
    if (!anyActive) togglePlaying(false)
  }, [anyActive])

  // Stop if no change from previous step
  useEffect(() => {
    if (!playing || cellActions.position === 0) return
    const prevCells = cellActions.history[cellActions.position - 1]
    for (let i = 0; i < gridSize; i++)
      for (let j = 0; j < gridSize; j++)
        if (cells[i][j] !== prevCells[i][j]) return
    togglePlaying(false)
  }, [cells])

  // Stop if modal opened
  useEffect(() => {
    if (modalState !== ModalState.none) togglePlaying(false)
  }, [modalState])

  const countNeighbours = useMemo(() => {
    if (wrapEdges) return countNeighboursWrap
    else return countNeighboursNoWrap
  }, [wrapEdges])
  ...
  const tick = () =>
    setCells(cells =>
      cells.map((row, i) =>
        row.map((cell, j) =>
          nextCellState(cell, countNeighbours(cells, gridSize, i, j))
        )
      )
    )
  ...
  return (
    <SizeContext.Provider value={[gridSize, changeGridSize]}>
      ...
      <div className={classes.gridContainer}>
        <GolGrid cells={cells} toggleCell={clickCell} scale={gridScale} />
      </div>
      ...
      <FloatingBoardActions playing={playing} handleRandomize={randomize} />
      ...
      />
      <AboutModal open={modalState === ModalState.about} onClose={closeModal} />
