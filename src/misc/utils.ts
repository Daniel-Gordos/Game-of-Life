...
export const useToggle = (initial:boolean | (() => boolean)): [boolean, ((val?: boolean) => void)] => {
...
type HistoryReturn<S> = [
  S,
  (val: HistorySetter<S>) => void,
  HistoryState<S>
]

export const useStateHistory = <S>(initial: S | (() => S), capacity: number): HistoryReturn<S> => {
